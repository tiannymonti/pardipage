...
{
    press_select_all2 = false;
    press_deselect_all2 = false;
    $('#windturbine.checkbox').on('ifToggled', function(event){
        if (!($("input.flat:checked").length == $("input.flat:checkbox").length) && press_select_all2){
            return;
        }
        if (!($("input.flat:checked").length == 0) && press_deselect_all2){
            return;
        }
        checked_ids = [];
        column_header = [];
        ...
        if(checked_ids.length!=0){
            ...
            */
            $.post('/mysql/exportWindTable.php', function(phpdata){
                processJSON(phpdata);
            });
        }
        else
        {
            var wrapper_element = document.getElementById('datatable2_wrapper');
            if (wrapper_element !== null){
                $("#datatable2_wrapper").css("opacity",0);
                $("div#datatable2_wrapper").remove();
                myTable.destroy();
            }
            if (press_deselect_all2){
                press_deselect_all2 = false;
            }
        }
    });
...
function createTable(headers,dataSet){
    $("div#datatable2_wrapper").remove();
    var tabledef = '<table id="datatable2" class="table table-striped table-bordered bulk_action" style="opacity:0;"> </table>';
    $("div#refreshTable2").append(tabledef);
    ...
    $("#datatable2_wrapper").css("opacity",100);
    $("table#datatable2").css("opacity",100);
    //This comment
    drawTable();
